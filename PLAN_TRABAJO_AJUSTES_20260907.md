...
| 14 | El **indicador de encuestador** no aparece; probado con 3 usuarios distintos y siempre da cero. | `app/api/reports/route.ts` (cálculo de `summary.totalSurveyors`) + `components/reports/summary-content.tsx` (tarjeta "Total Encuestadores"). **Nota:** en `dev-testing-main` este módulo ya se rehízo por completo (ya no es el `app/reports/page.tsx` con datos de ejemplo de `main`) y el "siempre en cero" del acta original ya no ocurre. Sí se encontró y corrigió un bug relacionado: cuando un encuestador tenía respuestas mezclando el flujo por asignación (`assignment_id`) y el flujo web autenticado directo (sin asignación), se agrupaba con dos identificadores distintos (`surveyors.id` vs `surveyors.user_id`) y aparecía **duplicado** — inflando "Total Encuestadores" y repartiendo mal sus indicadores de rendimiento (afecta también #21/#28/#29). Corregido en `app/api/reports/route.ts` (commit de este ítem). | **Alta** (bug de datos) | M/L | ✅ Corregido (pendiente validar en ambiente con datos reales) |
...
| # | Ajuste | Módulo/Componente probable | Prioridad | Complejidad | Estado |
|---|---|---|---|---|---|
| 28 | No aparecen los indicadores de los encuestadores. | `app/reports/page.tsx` (tab `performance`, tarjetas + tabla "Rendimiento por Encuestador"). **Confirmado con captura:** los indicadores sí aparecen (Total Respuestas, Encuestadores, Tasa Global, tabla por encuestador), pero la tarjeta "Encuestadores" decía **"con asignaciones"** cuando en realidad `surveyorPerformance` ya viene filtrado a encuestadores **con respuestas** (`totalRegistros > 0`, ver ítem #14) — etiqueta engañosa, corregida a "con respuestas". | **Alta** | M | 🟡 Parcial — etiqueta corregida; falta confirmar con captura de la tabla completa (todas las filas) si los números por encuestador cuadran contra el total |
| 29 | "Rendimiento por encuesta" no cuadra: muestra tasa 100% cuando el resumen general da 37,5% → **descuadre de cálculo entre vistas**. | `app/reports/page.tsx` (tab `performance` vs `summary`) — revisar fórmula de tasa de respuesta. **Revisado:** tanto Resumen como Rendimiento leen la misma `summary.tasaRespuestasEfectivas` (`app/api/reports/route.ts`), calculada una sola vez sobre el total de respuestas — ya no hay dos fórmulas distintas que puedan descuadrar a nivel global. El descuadre original puede haber sido el mismo bug de encuestador duplicado corregido en #14 (partía sus datos en dos filas con tasas parciales distintas), o referirse a "Rendimiento por Encuesta" (tabla `surveyPerformance`, no por encuestador) — pendiente confirmar con captura de esa tabla específica. | **Alta** (bug de datos, credibilidad del reporte) | M | 🟡 Parcial — causa raíz más probable ya corregida en #14; falta confirmar con foto de "Rendimiento por Encuesta" |
