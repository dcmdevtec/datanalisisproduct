...
| 18 | Solo permite personalizar (cambiar color) **parte** de la gráfica. | `components/reports/question-chart.tsx` + `question-card.tsx`. Pasó por dos rondas: primero solo se coloreaba la primera barra; luego (fix anterior) se generaban tonos del color elegido para toda la gráfica. El cliente aclaró (08/09/2026): "si selecciono la barra, la barra que seleccione es la que le cambio el color no todas" — quiere personalización **por barra/porción**, no un color global. Corregido de nuevo: ahora se hace clic en una barra/porción (o su fila en la tabla) para seleccionarla, y el color elegido en la paleta se aplica **solo a esa**; el resto conserva su color (por defecto o su propio override). Los colores se guardan por pregunta en `settings.colorOverrides` (persiste igual que el resto de la config del informe). | Baja | S/M | ✅ Corregido (2da ronda, según aclaración del cliente) |
...
## 6. Reunión de seguimiento 09/09/2026 — acta completa por temas

Documento más detallado y organizado por 10 bloques temáticos (creación de encuestas, asignación, identificación, PDF, gráficas, rendimiento, respuestas individuales, mapas, audios, sesión). Se revisó ítem por ítem contra el código y contra lo ya resuelto en las secciones 1 y 5 de este plan. Los ítems ya cubiertos ahí no se repiten acá.

| # | Ajuste | Diagnóstico / solución | Estado |
|---|---|---|---|
| B1 | Descripción de sección: orden título encuesta → sección → descripción; no mostrar descripción general de la encuesta en esa vista. | Verificado en código (`app/preview/survey/page.tsx`): ya estaba corregido en una ronda anterior, con comentario fechado 07/09/2026 confirmándolo. | ✅ Ya estaba corregido |
| B2 | Guardar lo alcanzado a responder al abandonar (no solo limpiar el borrador local). | Se extrae `normalizeSurveyAnswers()` a `lib/survey-preview-data.ts` (reutilizable) y `handleAbandon` ahora manda las respuestas del borrador al backend, que las inserta ligadas a la respuesta de abandono. | ✅ Corregido |
| B3 | Mostrar encuestas en "Prueba" a los encuestadores (asignadas y dentro de rango de fechas). | `app/api/portal-encuestador/surveys/route.ts` y `assignments/[id]/route.ts` excluían todo lo que no fuera `status="active"` — ahora permiten también `"draft"`. | ✅ Corregido |
| B4 | Saltos lógicos por respuesta (favorable/desfavorable/no conoce/descalificación) en la web. | Ya cubierto por A2 (autosave del salto de sección) — es el mismo bug raíz. | ✅ Corregido (ver A2) |
| B5 | Etiquetas de escalas de clasificación mal ubicadas (ej. escala 1-20). | En rangos grandes (>10) los valores se acomodan en una grilla de varias filas — la etiqueta min/max de ancho completo se descuadraba en cualquier rango cuya última fila no llenara las 10 columnas. Ahora usa la misma grilla que los valores, con cada etiqueta en la celda exacta de su valor. | ✅ Corregido |
| B6 | Fijar enunciado + encabezados de matrices grandes al hacer scroll. | Los encabezados de columna ya quedaban fijos (ronda anterior). Faltaba el enunciado de la pregunta (vive fuera de la tabla) — ahora también queda sticky para matrices de más de 8 filas. | ✅ Corregido |
| B7 | Carga múltiple de opciones en matriz tipo "lista desplegable". | Ya existía (botón "Múltiples" + textarea por columna en `question-editor.tsx`) — verificado, sin cambios necesarios. | ✅ Ya estaba implementado |
| B8 | Responsive de "Desde"/"Hasta" en pantallas pequeñas. | Revisado dos veces (Reportes y Dashboard, mismo patrón en ambos): el contenedor ya es `flex-col` en móvil y cada input ya es ancho completo — no se encontró el bug en el código. | ⏳ Necesita captura del ancho exacto donde se ve mal — no se pudo reproducir por lectura de código |
| B9 | Desagrupar jerarquía fija + grupos personalizados por proyecto (universo completo al asignar). | Ya cubierto por A1 (`survey-hierarchy-assignment.tsx`). | ✅ Corregido (ver A1) |
| B10 | Cada supervisor/coordinador solo debe ver su grupo asignado dentro de ese proyecto (no su equipo global). | `app/api/reports/route.ts`: cuando el propio supervisor/coordinador logueado mira una encuesta con asignación propia para ese proyecto, ahora ve exactamente ese equipo (antes se unía con su equipo global). Un admin inspeccionando a mano sigue viendo la unión. | ✅ Corregido |
| B11 | Código interno obligatorio, mismo ID que la encuesta. | Ya cubierto por A3 — el acuerdo confirma que el diseño elegido (mismo campo) fue el correcto. | ✅ Construido (ver A3) |
| B12 | Mostrar nombre + código en todas las vistas/reportes/PDF. | Se agrega en: listado de Encuestas (tabla y tarjetas), encabezado de Reportes cuando se llega por "Ver reporte", PDFs de reportes, y el PDF de la encuesta misma. | ✅ Corregido |
| B13 | Usar el código interno en el enlace de la encuesta. | Se complementa (no se reemplaza, para no romper links ya compartidos): `/encuesta/ENC-2026-0001` funciona igual que `/encuesta/<uuid>` — `/api/surveys/[id]` detecta cuál de los dos le llegó. El modal "Compartir Encuesta" (input, copiar, QR, WhatsApp) usa el código cuando existe. | ✅ Corregido |
| B14 | Formato original de enunciados + reglas de saltos en el PDF de la encuesta. | Ya cubierto en la sección 1 (#10/#11). | ✅ Corregido |
| B15 | Evitar que el título de una sección quede separado de su primera pregunta en el PDF. | El header de sección y su primera pregunta se agrupan en un bloque `break-inside-avoid` — Chromium los trata como unidad indivisible al paginar. | ✅ Corregido |
| B16 | Número absoluto + % en todas las gráficas. | Barras ya lo mostraban (ronda anterior). Torta/anillo solo lo mostraban en tooltip/leyenda — ahora también en la etiqueta fija sobre cada porción. | ✅ Corregido |
| B17 | Un decimal en todos los %, colores individuales, gráficas en el PDF, nombre+código en PDF de reportes, centrar/ajustar PDF. | Ya cubiertos en la sección 1 y en B12. | ✅ Corregido (ver arriba) |
| B18 | Zona horaria de "Actividad por hora" y de los indicadores "Hoy" del mapa de encuestadores. | El fix de Bogotá (ronda anterior) solo cubría Resumen/Rendimiento de Reportes. Se extendió a: exports Excel, reporte público con token, reporte público legado, y — el más importante — el cálculo de "medianoche de hoy" en `app/api/tracking/route.ts`, que usaba la hora del servidor y arrancaba el día con horas de la tarde/noche anterior coladas de más. Nuevo helper compartido `lib/bogota-time.ts`. | ✅ Corregido |
| B19 | Aparición intermitente de indicadores de encuestadores. | No se pudo reproducir por lectura de código — necesita una captura/video del momento exacto en que un indicador desaparece y vuelve a aparecer. | ⏳ Necesita captura/video |
| B20 | Desfase "Rendimiento por encuestador" vs. resumen general (ej. 26 registros en resumen, 4 en rendimiento). | Se revisó a fondo la cadena de resolución de encuestador (assignment_id → survey_surveyor_zones → surveyor_id) — se ve correcta en código, y ya tiene el fix de #14/#40 de la sección 1. Reproducido en vivo el 08/09 con 0 encuestadores en un caso puntual; quedó pendiente que el cliente confirme si el encuestador de prueba estaba realmente asignado en Asignación para esa encuesta — sin esa confirmación no se pudo aislar más el problema. | ⏳ Necesita confirmación del cliente sobre la asignación real, o un caso reproducible con encuestador+encuesta puntuales |
| B21 | Reiniciar diariamente los indicadores operativos (Total, efectivas, primera/última hora). | Mismo fix que B18 — el bug de zona horaria en `api/tracking/route.ts` ERA la causa de que "hoy" no arrancara en cero. Se agrega también "fin de la última encuesta" (antes solo existía "inicio de la primera"). | ✅ Corregido |
| B22 | Orden ascendente/descendente en todas las tablas de Reportes, no solo Rendimiento por Encuestador. | Nuevo componente genérico `components/reports/sortable-table.tsx`. Se aplica a "Rendimiento por Encuesta" y "Comparativa por Zona", que eran grillas planas sin ordenar. La tabla cruzada (crosstab) se deja igual a propósito — no tiene sentido ordenarla como tabla simple. | ✅ Corregido |
| B23 | Editar respuestas de matriz para todos los tipos de celda (no solo radio/checkbox). | Se descubrió que la vista de SOLO LECTURA de los otros tipos (select/texto/número/rating/ranking) también estaba rota (mostraba un grid de checkmarks que nunca encontraba nada). Corregido leer y editar cada tipo con su formato real — incluye nuevos campos `matrixColOptions`/`matrixRatingScale` en la API. | ✅ Corregido |
| B24 | Limitar edición a las opciones configuradas. | Ya cubierto en la sección 1 (#26) — ahora extendido también a celdas "select" de matriz (ver B23). | ✅ Corregido |
| B25 | Mapa: graficar todos los tipos de resultado (no solo efectiva/descalificada). | Causa raíz real: al abandonar o reportar una incidencia nunca se mandaba la ubicación GPS al servidor — esas respuestas quedaban siempre sin coordenadas y por eso nunca podían aparecer en el mapa. Se captura la ubicación (mejor esfuerzo) en ambos flujos. | ✅ Corregido |
| B26 | Corregir la captura del mapa en el PDF (aparece desplazado). | La copia oculta del mapa vivía en `left: -9999px` — muy lejos de cualquier coordenada real de viewport, lo que podía hacer que Leaflet (transforms internos) y html2canvas (coordenadas de viewport) calcularan un offset distinto entre sí. Se deja en una posición real (0,0), oculta con `opacity:0`/`pointer-events:none` en vez de un corrimiento extremo. | ✅ Corregido |
| B27 | Generar los PDF de mapas en horizontal. | Ya estaba implementado (ítem #33 de la sección 1). | ✅ Ya estaba corregido |
| B28 | Reducir tamaño de puntos del mapa con volumen alto. | El radio (fijo en píxeles, no cambia con el zoom) ahora se reduce según cuántos puntos hay que dibujar a la vez. | ✅ Corregido |
| B29 | Mostrar correctamente el encuestador de cada punto (algunos salían "sin asignar"). | Ya resuelto por el mismo fix de resolución de encuestador de la sección 1 (#14/#31) — verificado en código, ese camino ya usa el id canónico. | ✅ Ya estaba corregido |
| B30 | Agregar "Ver encuesta" desde un punto del mapa (a la respuesta individual, no al resumen). | El botón del popup pasó de dibujar la ruta GPS del encuestador (redundante con el control "Ver ruta" del panel) a abrir el mismo modal de detalle de "Respuestas Individuales". | ✅ Corregido |
| B31 | Guardar y consultar histórico de rutas por encuestador (hoy/ayer/fecha anterior). | Ya implementado en una ronda anterior (selector de fecha en el mapa de Encuestadores). | ✅ Ya estaba implementado |
| B32 | Mejorar la ubicación de los filtros del mapa (quedan atravesados al ampliar). | No se pudo reproducir por lectura de código — necesita una captura de la pantalla ampliada/pantalla completa donde ocurre. | ⏳ Necesita captura |
| B33 | Asociación de audios con cada encuesta/intento — varios audios previos sin quedar claro a cuál pertenecen. | Causa raíz: los clips "antes de iniciar" se armaban con una ventana FIJA de 30 min hacia atrás, sin importar si en esos 30 min hubo un intento anterior del mismo turno (una encuesta abandonada) — sus clips quedaban mezclados como si fueran de la encuesta actual. Se acota la ventana al final del intento anterior más reciente, si existe. | ✅ Corregido |
| B34 | Un único audio continuo, o al menos bien relacionados y ordenados si se mantienen separados. | Ya se fusionan en un solo archivo al descargar (ronda anterior, `lib/audio-merge.ts`) — se mantienen separados solo en el visor web (con orden cronológico y ahora con el fix de B33). Decisión de producto sobre fusionar también el visor, no un bug. | 🟡 Decisión de producto — funcionalidad base ya existe |
| B35 | Verificar encuestas configuradas para grabación que aparecen sin audio. | No se pudo reproducir sin datos reales — necesita un caso puntual (encuesta + encuestador) para poder rastrear por qué falta. | ⏳ Necesita caso reproducible |
| B36 | Repositorio jerárquico de audios (encuesta → fecha → encuestador) con selección por casillas. | Se reestructura la pestaña Audios en un árbol Fecha → Encuestador → grabación, colapsable, con casillas en cascada. El ZIP acepta ahora una lista exacta de ids seleccionados (`recordingIds`), no solo un filtro de fecha/encuestador de a uno. | ✅ Corregido |
| B37 | Filtro de fechas del repositorio de audios conectado al filtro ya existente. | El árbol nuevo (B36) agrupa por fecha directamente — ya no hace falta un selector de fecha aparte, cada fecha es un nodo del árbol. | ✅ Corregido (rediseñado) |
| B38 | Probar reproducción offline del formato de audio actual. | Es una tarea de prueba manual del equipo (Carlos, según el acta), no un cambio de código. | 🟡 Pendiente de prueba manual del equipo |
| B39 | Restringir el cierre de sesión del encuestador — solo supervisor/admin puede cerrarla remotamente. | Feature nueva completa: mientras la jornada está activa, el encuestador no puede cerrar su propia sesión; nuevo botón "Cerrar sesión remota" en Encuestadores → Geolocalización, para admin/supervisor. Requiere una migración SQL nueva (`db/migrations/2026-09-09_add_surveyor_force_logout.sql`). | ✅ Corregido |

**Resumen del estado tras esta ronda:** de 39 ítems de esta acta de seguimiento, 32 quedaron corregidos o confirmados como ya resueltos, 2 son decisiones de producto o tareas de prueba manual (no bugs), y 5 necesitan una captura/video puntual o una confirmación de datos del cliente para poder seguir (B8, B19, B20, B32, B35) — no se pudieron diagnosticar más a fondo por lectura de código sola, sin navegador ni base de datos en vivo en este entorno.

**Dos migraciones SQL pendientes de correr manualmente** (no se aplican solas — ver `db/migrations/README.md`):
- `db/migrations/2026-09-08_add_survey_code.sql` (ID único de encuesta)
- `db/migrations/2026-09-09_add_surveyor_force_logout.sql` (cierre de sesión remoto)
