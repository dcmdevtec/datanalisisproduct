...
| # | Ajuste | Módulo/Componente probable | Prioridad | Complejidad | Estado |
|---|---|---|---|---|---|
| 22 | No toma el **nombre del encuestado** en la tabla. | `components/reports/individual-responses-tab.tsx` | **Alta** | S/M | ✅ Ya estaba implementado (`respondentName` se muestra en fila y detalle; fallback "Sin asignar" si no hay) |
...
| 25 | Las **tablas matriz no se pueden editar** en la respuesta (no aparece la opción). | `components/reports/individual-responses-tab.tsx`. Confirmado en código: el botón de editar respuesta excluye explícitamente `matrix` ("necesita su propio editor, fuera de alcance"). Requiere construir un editor de celdas de matriz dedicado — no es un ajuste chico. | Media | M | ⏳ Diagnosticado, no implementado — requiere ronda dedicada (UI de edición celda por celda) |
| 26 | Al editar una pregunta de respuesta, debería limitarse **solo** a las opciones creadas para esa pregunta (evitar edición libre/inconsistente). | `components/reports/individual-responses-tab.tsx` + `app/api/reports/individual/route.ts`. Confirmado en código: hoy la edición es un `<Textarea>` de texto libre para cualquier tipo de pregunta — no hay selector restringido a las opciones válidas. Requiere que el API de detalle empiece a mandar las opciones de cada pregunta (hoy no las incluye) y un selector/checkbox en el frontend según el tipo. | Media | M | ⏳ Diagnosticado, no implementado — requiere cambio de API + UI |
