...
export async function exportSummary(data: any, period: string) {
  ...
  wsKpi.addRow(["Encuestas Efectivas", summary?.efectivas ?? 0])
  wsKpi.addRow(["Incidencias", summary?.incidencias ?? 0])
  wsKpi.addRow(["Encuestas Abandonadas", summary?.abandonadas ?? 0])
  wsKpi.addRow(["Tasa de Respuestas Efectivas (%)", summary?.tasaRespuestasEfectivas ?? 0])
