    ...
    setError(null)
      ...
      const surveyData = {
       ...
       if (isEditMode && surveyId) {
         ...
         // Para edición, eliminar registros existentes y luego insertar nuevos (igual que creación)
         console.log("🗑️ Limpiando registros existentes para reemplazar...")
         ...
         const { error: deleteSectionsError } = await supabase.from("survey_sections").delete().eq("survey_id", surveyId)
         if (deleteSectionsError) {
           console.error("❌ Error al eliminar secciones existentes:", deleteSectionsError)
           throw deleteSectionsError
         }
         console.log("✅ Secciones existentes eliminadas")

         // Eliminar asignaciones encuestador-zona existentes
        ...
        // Insert sections and their nested questions (TANTO PARA CREACIÓN COMO EDICIÓN)
        console.log(`📝 Insertando ${sections.length} secciones...`)
          ...
          // Validar que no se esté intentando modificar un ID existente
          if (section.id && section.id !== '') {
            console.log(`⚠️ ADVERTENCIA: Sección "${section.title}" ya tiene ID: ${section.id}`)
            console.log(`⚠️ No se debe modificar el ID de una sección existente`)
          }
          ...
          if (questionsToInsert.length > 0) {
            ...
            if (questionsError) {
              ...
              throw new Error(`Error al crear las preguntas de la sección "${section.title}": ${questionsError.message}`)
            }
            console.log(`✅ ${questionsToInsert.length} preguntas insertadas correctamente en la sección "${section.title}"`)
          } else {
        ...
        for (const zoneId of settings.assignedZones || []) {
          ...
          for (const surveyorId of surveyorsForZone) {
            if (surveyorId && zoneId) { // Validar que ambos IDs existan
    ...
    <ClientLayout>
      <DashboardLayout>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        ...
                        <DragOverlay>
                          {sections.find((s) => s.id === (sensors[0] as any)?.active?.id) ? (
                            <div className="p-4 border rounded-md bg-white shadow-lg opacity-80">
                              <h3 className="text-lg font-semibold">
                                {sections.find((s) => s.id === (sensors[0] as any)?.active?.id)?.title || "Sección"}
                              </h3>
                            </div>
                          ) : null}
                        </DragOverlay>
                      ...
                      <div className="border-2 border-dashed border-muted rounded-lg p-6 text-center hover:border-primary/50 transition-colors">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <div>
                            <h3 className="font-medium">Agregar nueva sección</h3>
                  ...
                  <div className="space-y-4">
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        >
                          <Users className="h-4 w-4" />
                          {settings.assignedUsers?.length > 0 ? "Editar" : "Asignar"} Encuestadores
                        </Button>

                        {settings.assignedUsers && settings.assignedUsers.length > 0 ? (
                          <div className="space-y-2">
                            ...
                            {settings.assignedUsers.length > 3 && (
                              <p className="text-xs text-muted-foreground text-center">
                                +{settings.assignedUsers.length - 3} más
                              </p>
                            )}
                          </div>
                        ) : (
        ...
        <AssignSurveyorsModal
          isOpen={showAssignSurveyorsModal}
          onClose={() => setShowAssignSurveyorsModal(false)}
          currentAssignedSurveyorIds={settings.assignedUsers || []}
