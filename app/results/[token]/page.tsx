...
function QuestionCard({ q, index }: { q: QuestionBreakdown; index: number }) {
  ...
  const isText = q.textAnswers.length > 0
  const isPie = isChoice && q.choices.length <= 6
  const isBar = isChoice && q.choices.length > 6
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5 mb-4">
          ...
          <div className="w-full md:w-1/2 space-y-2">
            {q.choices.map((c, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-400 w-10 text-right tabular-nums">{c.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {isBar && (
        <div style={{ height: Math.max(180, q.choices.length * 36) }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart layout="vertical" data={q.choices} margin={{ left: 8, right: 24, top: 4, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 11 }} />
              <YAxis type="category" dataKey="label" width={160} tick={{ fontSize: 11 }} />
              <Tooltip formatter={(v: any, _: any, p: any) => [`${v} (${p.payload.percentage}%)`, "Respuestas"]} />
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{meta.surveyTitle}</h1>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                ...
                <p className="text-3xl font-bold tabular-nums text-gray-900">{summary.tasaRespuestasEfectivas}%</p>
              </div>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                <div className="flex items-center gap-1.5 text-xs font-medium text-gray-500 mb-2">
