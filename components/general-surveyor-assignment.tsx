...
interface GeneralSurveyorAssignmentProps {
  allSurveyors: Surveyor[]
  assignedSurveyorIds: string[]
  onAssignmentChange: (newAssignedSurveyorIds: string[]) => void
}

export function GeneralSurveyorAssignment({
  allSurveyors,
  assignedSurveyorIds,
  onAssignmentChange,
}: GeneralSurveyorAssignmentProps) {
  ...
  return (
    <div className="border rounded-md p-4 space-y-3 bg-background">
      <h4 className="text-md font-semibold flex items-center gap-2">
        ...
        <span className="text-primary">Encuestadores Generales</span>
      </h4>
      <p className="text-sm text-muted-foreground">
        Encuestadores con acceso a todas las zonas seleccionadas
      </p>
        ...
        <PopoverTrigger asChild>
          ...
          >
            {selectedSurveyors.length > 0 ? (
              ...
              "Asignar encuestadores generales..."
            )}
